...
class User:
    """Represent a user that can buy a cinema seat"""
    def __init__(self, name):
        self.name = name

    def buy(self, seat, card):
        """Buy ticket if the card is valid"""
        if seat.is_free():
    ...
    def validate(self, price):
        ...
        if result:
            ...
            if balance >= price:
                ...
                """[int(balance - price), self.number, self.cvc])
                connection.commit()
                connection.close()
                return True
...
class Ticket:
    """Represent a cinema ticket purchase by a user"""
    ...
    def to_pdf(self):
        """Crate a ticket with PDF format"""
        ...
        pdf.set_font(family="Time", style="B", size=14)
        pdf.cell(w=100, h=25, txt="Name", border=1)
        pdf.set_font(family="Time", style="", size=12)
